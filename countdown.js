...
function getDateInLocale(timeZone, date) {
    return new Date(date.getTime() - (date.getTimezoneOffset() - getTimezoneOffset(date, timeZone)) * 60 * 1000);
}

function getResult(date, timeZone) {
    date = getDateInLocale(timeZone, date);
    ...
    return {
        until: msToTime(date.getTime() - new Date().getTime()),
        your: {zone: yourTimeZone, time: yourTime},
        their: {zone: timeZone, time: theirTime}
    };
}

let interval = null;

function run(dateValue, timeValue, zoneValue, titleValue) {
    function innerRun(dateValue, timeValue, zoneValue, titleValue) {
        let date = new Date(dateValue + " " + timeValue);
        let result = getResult(date, zoneValue);
        document.getElementById("eventTitle").innerText = titleValue;
        document.getElementById("yourTime").innerText = result.your.time;
        document.getElementById("yourZone").innerText = result.your.zone;
        document.getElementById("theirTime").innerText = result.their.time;
        document.getElementById("theirZone").innerText = result.their.zone;
        document.getElementById("untilHours").innerText = "" + result.until.hours;
        document.getElementById("untilMinutes").innerText = "" + result.until.minutes;
        document.getElementById("untilSecond").innerText = "" + result.until.seconds;
        let shareUrl = window.location.href.split('?')[0] + "?share=" + btoa(JSON.stringify({d: dateValue, h: timeValue, z: zoneValue, t: titleValue}));
        document.getElementById("shareUrl").innerText = shareUrl;
        document.getElementById("shareUrl").href = shareUrl;
    }

    innerRun(dateValue, timeValue, zoneValue, titleValue);
...
function submitClicked(event) {
    ...
    try {
        ...
        run(dateValue, timeValue, zoneValue, titleValue);
...
function start() {
    ...
    try {
        if(share) {
            ...
            run(share.d, share.h, share.z, share.t);
            document.getElementById("result").hidden = false;
            document.getElementById("submitForm").hidden = true;
        }
    } catch (e) {
...
function addOptions() {
    ...
    options.forEach(option => {
        ...
        document.getElementById("zones").appendChild(optionElement);
    })
}
